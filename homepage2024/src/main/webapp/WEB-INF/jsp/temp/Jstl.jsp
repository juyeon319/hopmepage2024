...
<%-- 					<c:import url="/temp/selectList.do" charEncoding="utf-8" /> --%>
			...
			<c:when test="${step eq '5' }">
				<li>
					<h3>c:choose, c:when, c:otherwise Tag : 조건문 중의 하나 <br>
						JAVA에서 사용하는 If, else if, else와 비슷함
					</h3>
					<h4>기본문법 : <br>
						&#60; c:choose&#62;<br>
							&#60;when test="조건문"&#62; 조건 true에 대한 내용 &#60;/c:when&#62;<br>
			...
			<c:when test="${step eq '6' }">
				<li>
					<h3>c:forEach Tag : 기본 반복 태그, 고정 된 횟수 또는 초과 수집 동안 중첩된 본문 내용을 반목 <br>
						JAVA에서 사용하는 If, else if, else와 비슷함
					</h3>
					<h4>기본문법 : <br>
		...
		<c:when test="${step eq '19' }">
			<li>
				<h3>fn:trim() : 문자열의 양쪽 끝에서 공백을 제거(문자열 사이의 공백은 사라지지않음)</h3>
